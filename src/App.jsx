...
const emptyData = () => ({
  panels: [], loops: [], nacs: [], devices: [], pumpDevices: [], gasDetectors: [], redeDispositivos: [],
...
function complementaresTrackableItems(data) {
  ...
  (data.redeDispositivos || []).forEach((r) => {
    ...
    items.push({
      id: r.id, category: 'redeDispositivos', panelId: r.panelId || null,
      title: `${r.tipoLabel || 'Rede'}${r.etiqueta ? ` — ${r.etiqueta}` : ''}${panel ? ` (${panel.name})` : ''}`,
      address: null, modelo: r.modelo || '',
      meta: 'Dispositivos Complementares',
      nextMaintenance: r.nextMaintenance, lastMaintenance: r.lastMaintenance,
      nextInspection: r.nextInspection, lastInspection: r.lastInspection,
      icon: Zap, photo: null,
    });
  });
  return items;
...
function Workspace({ client, onUpdateClient, onSwitchClient }) {
  ...
  function submitRedeDispositivo(mode, initial, values) {
    updateData((prev) => {
      const row = {
        panelId: values.painelId, tipo: values.tipo,
        etiqueta: (values.etiqueta || '').trim(), modelo: (values.modelo || '').trim(),
        nextMaintenance: values.nextMaintenance || '',
      };
      if (mode === 'create') {
        return { ...prev, redeDispositivos: [...(prev.redeDispositivos || []), { id: uid(), ...row }] };
      }
      return {
        ...prev,
        redeDispositivos: (prev.redeDispositivos || []).map((r) => (r.id === initial.id ? { ...r, ...row } : r)),
      };
    });
  }

  function deleteRedeDispositivo(id) {
    updateData((prev) => ({ ...prev, redeDispositivos: (prev.redeDispositivos || []).filter((r) => r.id !== id) }));
    setConfirmState(null);
  }
      ...
      (async () => {
        try {
          ...
          await createAtendimento({
            dispositivoId: values.deviceId, clienteId: client.id, falha: values.falha, status: (values.status || 'Aguardando').toLowerCase(),
    ...
    if (['devices', 'nacs', 'gasDetectors'].includes(category)) {
      (async () => {
        try {
          ...
          await createAtendimento({
            dispositivoId: id, clienteId: client.id, falha: values.falha, status: (values.status || 'Resolvido').toLowerCase(),
      ...
      (async () => {
        try {
          ...
          await createInspecao({
            dispositivoId: id, clienteId: client.id, tecnico: values.technician || '',
          ...
          <div className="flex flex-col gap-4">
            ...
            {view === 'complementares' && (
              <ComplementaresView data={data} canEdit={canEdit}
                ...
                onSubmitRede={submitRedeDispositivo}
...
function ComplementaresView({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {tab === 'rede' && (
        <div key="rede" className="fade-in-up">
          <RedeList data={data} canEdit={canEdit} onSubmit={onSubmitRede} onDelete={onDeleteRede} />
        </div>
      )}
      {tab === 'baterias_painel' && (
...
function RedeForm({ initial, panels, onSubmit, onCancel }) {
  const [v, setV] = useState({
    painelId: initial?.panelId || (panels[0]?.id || ''),
    tipo: initial?.tipo || 'rede_conversor',
    etiqueta: initial?.etiqueta || '',
    modelo: initial?.modelo || '',
    nextMaintenance: initial?.nextMaintenance || '',
  });
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (v.painelId) onSubmit(v); }}>
      <Field label="Painel *">
        <select className={inputCls} value={v.painelId} onChange={(e) => setV({ ...v, painelId: e.target.value })} required>
          <option value="">Selecione...</option>
      ...
      <Field label="Tipo *">
        <select className={inputCls} value={v.tipo} onChange={(e) => setV({ ...v, tipo: e.target.value })}>
          {Object.entries(REDE_TIPOS).map(([k, label]) => <option key={k} value={k}>{label}</option>)}
        </select>
      </Field>
      <Field label="Etiqueta / identificação"><input className={inputCls} value={v.etiqueta}
        onChange={(e) => setV({ ...v, etiqueta: e.target.value })} placeholder="Ex.: Conversor rack CPD" /></Field>
      <Field label="Modelo"><input className={inputCls} value={v.modelo} onChange={(e) => setV({ ...v, modelo: e.target.value })} /></Field>
      <Field label="Próxima inspeção"><input type="date" className={inputCls} value={v.nextMaintenance}
        onChange={(e) => setV({ ...v, nextMaintenance: e.target.value })} /></Field>
      <FormActions>
        <Button variant="secondary" type="button" onClick={onCancel}>Cancelar</Button>
        <Button variant="primary" type="submit">Salvar</Button>
      </FormActions>
...
function RedeList({ data, canEdit, onSubmit, onDelete }) {
  ...
  return (
    <div className="flex flex-col gap-3">
            ...
            <div key={r.id} className="rounded-lg p-3.5 flex flex-col gap-2" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
              <div className="flex items-center justify-between gap-2">
                <span className="font-medium text-sm" style={{ color: 'var(--text-primary)' }}>
                  {REDE_TIPOS[r.tipo] || 'Rede'}{r.etiqueta ? ` — ${r.etiqueta}` : ''}
                </span>
                {canEdit && (
                  <div className="flex gap-1 flex-shrink-0">
                    <IconButton title="Editar" onClick={() => setModalState({ mode: 'edit', initial: r })}><Pencil size={14} /></IconButton>
                    <IconButton title="Excluir" danger onClick={() => onDelete(r.id)}><Trash2 size={14} /></IconButton>
                  </div>
                )}
              </div>
              <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                Painel: {panelName(r.panelId)}{r.modelo ? ` · ${r.modelo}` : ''}<br />
                {r.lastInspection ? `Última inspeção: ${formatDateBR(r.lastInspection)}` : 'Sem inspeção registrada'}
                {r.nextInspection ? ` · Próxima: ${formatDateBR(r.nextInspection)}` : ''}
              </div>
            </div>
          ))}
        </div>
      ...
      {modalState && (
        <Modal title={modalState.mode === 'create' ? 'Novo dispositivo de rede' : 'Editar dispositivo de rede'} onClose={() => setModalState(null)}>
          <RedeForm initial={modalState.initial} panels={data.panels || []}
            onSubmit={(values) => { onSubmit(modalState.mode, modalState.initial, values); setModalState(null); }}
            onCancel={() => setModalState(null)} />
        </Modal>
      )}
...
function ReportView({ data, client, filters, setFilters }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {reportTab === 'sdai' ? (
        <div key="sdai" className="grid grid-cols-2 md:grid-cols-4 gap-3 no-print fade-in-up">
        ...
        <div key="spci" className="grid grid-cols-2 md:grid-cols-4 gap-3 no-print fade-in-up">
