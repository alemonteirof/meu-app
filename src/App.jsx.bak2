...
/* ------------------------------------------------------------------ */
/* Supabase (banco de dados + login de usuÃ¡rios)                      */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* Todo o app jÃ¡ lÃª/grava dados atravÃ©s de window.storage.get/set/    */
...
/* caÃ­mos de volta para o localStorage do navegador (Ãºtil para testar */
...
/* ------------------------------------------------------------------ */
/* AutenticaÃ§Ã£o global + nÃ­veis de acesso (admin / operador / visualizador) */
/* ------------------------------------------------------------------ */
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (mode === 'login') {
        ...
        if (error) throw error;
        setInfo('Conta criada! Um administrador precisa liberar seu nÃ­vel de acesso antes que vocÃª possa usar o sistema plenamente.');
      }
    } catch (err) {
      setError(err.message || 'NÃ£o foi possÃ­vel entrar.');
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="login-content">
        <form onSubmit={handleSubmit} className="w-full max-w-sm rounded-2xl p-6" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
          <div className="flex flex-col items-center gap-3 mb-4">
            ...
            <h1 className="font-display text-lg font-semibold text-center" style={{ color: 'var(--text-primary)' }}>Centro de Controle de ManutenÃ§Ã£o</h1>
          </div>
          ...
          <button type="button" onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setError(''); setInfo(''); }}
            ...
            {mode === 'login' ? 'NÃ£o tem conta? Criar uma' : 'JÃ¡ tem conta? Entrar'}
          </button>
        </form>
      </div>
    </div>
...
function BrandLogo({ size = 18, boxSize = 36, rounded = true }) {
  ...
  return (
    <img src="/maj-logo-icon.png" alt="MAJ SoluÃ§Ãµes"
...
/* ------------------------------------------------------------------ */
/* ImportaÃ§Ã£o de base de dispositivos â€” mÃºltiplas marcas de painel     */
/* ------------------------------------------------------------------ */
...
const IMPORT_BRANDS = [
  {
    ...
    label: 'Hochiki / VES',
    models: [
      { value: 'generico', label: 'NÃ£o especificar / outro modelo', shortLabel: '', loops: null },
      { value: 'firenet-plus', label: 'FireNET Plus (Hochiki) / Elite RS (VES) â€” 1 ou 2 laÃ§os', shortLabel: 'FireNET Plus / Elite RS', loops: null },
      { value: 'latitude', label: 'FireNET L@titude (Hochiki) / L@titude (VES) â€” 2 a 16 laÃ§os', shortLabel: 'FireNET L@titude / L@titude', loops: null },
      { value: 'firenet', label: 'FireNET (Hochiki, descontinuado â€” sem equivalente VES) â€” 2 ou 4 laÃ§os', shortLabel: 'FireNET (descontinuado)', loops: null },
    ],
  },
  {
    value: 'notifier',
...
/* Mapeamento calibrado com os "Type Code Label" reais observados em exports do VeriFire
   Tools (relatÃ³rios de MÃ³dulos e Detectores). CorrespondÃªncia exata primeiro; quando o
   cÃ³digo nÃ£o Ã© reconhecido, cai para uma busca por palavra-chave. */
...
/* Modelo real do mÃ³dulo Notifier, a partir da coluna "FlashScan Type" do report.
   hasSub indica se o endereÃ§o tem sub-endereÃ§os (ex.: 056.01/056.02) â€” nesse caso, o
   MINI/DUAL MONITOR Ã© fisicamente um mÃ³dulo de dupla entrada (FDM-1); sem sub-endereÃ§o,
   Ã© um mÃ³dulo de entrada Ãºnica (FMM-101). */
function resolveNotifierModel(flashScanType, hasSub) {
  ...
  // PS MON (Power Monitor) refere-se a fontes auxiliares â€” ainda sem um cÃ³digo de
  // modelo especÃ­fico definido; por ora mostra o prÃ³prio FlashScan Type do report.
  return flashScanType;
}

/* ---- Hochiki / VES: relatÃ³rio "Device Labels" em .csv ---- */
...
/* ---- Hochiki / VES: relatÃ³rio do Loop Explorer 1 (.pdf) ----
   O LP1 exporta um PDF com layout de tabela sem linhas de grade: cada "cÃ©lula" Ã©
   posicionada por coordenadas, e o texto de cÃ©lulas longas (ex.: "Setting" com dois
   valores D/N, ou a descriÃ§Ã£o do local) quebra em linhas visuais que nÃ£o seguem a
   ordem de leitura simples. Por isso, em vez de interpretar linha por linha, lemos
   todas as palavras com suas coordenadas (x,y), localizamos os endereÃ§os (coluna da
   esquerda) e agrupamos as demais palavras numa "banda" vertical ao redor de cada
   endereÃ§o, atribuindo cada palavra Ã  coluna correta pela posiÃ§Ã£o horizontal. */
...
function parseLp1Report(words) {
  ...
  // ReconstrÃ³i linhas de texto simples (sÃ³ pra localizar os cabeÃ§alhos "Panel X" e "X - Loop N (...)")
  const lines = [];
  ...
  if (addrWords.length === 0) {
    throw new Error('NÃ£o encontrei endereÃ§os de dispositivos nesse PDF. Verifique se Ã© um relatÃ³rio do Loop Explorer 1 (LP1).');
  }
  ...
  // Palavras "tÃ©cnicas" das colunas que ficam entre Nome e Texto (Zona, Flags, AÃ§Ã£o,
  // Setting D/N e os Delays) â€” usadas sÃ³ para saber onde o Texto realmente comeÃ§a, jÃ¡
  // que nÃ£o usamos o conteÃºdo dessas colunas no app.
  ...
  // CombinaÃ§Ãµes de Flags realmente usadas neste tipo de relatÃ³rio (M=Alarme Geral,
  // P=PrÃ©-alarme, D=Modo Dia, N=Modo Noite, E=EmergÃªncia, S=SilenciÃ¡vel, R=ResetÃ¡vel,
  // L=Latching, O=SaÃ­da com atraso, F=Falha, A=SaÃ­da de PrÃ©-alarme, B=Bypass, H=Calor).
  // Usamos uma lista fechada (em vez de qualquer combinaÃ§Ã£o de letras) porque algumas
  // dessas letras coincidem com o inÃ­cio de textos reais do relatÃ³rio, como "AM"
  ...
  function extractLp1Label(tail) {
    ...
    // Flags â€” sÃ³ reconhece os combos realmente usados neste relatÃ³rio (ver lista acima).
    if (idx < tokens.length && LP1_KNOWN_FLAGS.has(tokens[idx])) idx++;
    // AÃ§Ã£o (pode ser "Fire", ou duas palavras como "Supervisory Al")
    while (idx < tokens.length && LP1_ACTION_WORDS.has(tokens[idx])) idx++;
    // Setting dia/noite â€” formatos "D2,50"/"N2,50" (com vÃ­rgula) ou "D150"/"N150" (sem
    // vÃ­rgula, usado por alguns modelos como ATG-EA). SÃ³ checa nas duas posiÃ§Ãµes logo
    // apÃ³s a AÃ§Ã£o (nunca mais adiante), pra nÃ£o arriscar cortar um cÃ³digo de local que
    // comece com D/N seguido de nÃºmeros (ex.: "N04" de uma coluna).
    if (idx < tokens.length && /^D\d+([.,]\d+)?$/.test(tokens[idx])) idx++;
    if (idx < tokens.length && /^N\d+([.,]\d+)?$/.test(tokens[idx])) idx++;
    // Delays â€” nÃºmeros soltos ("00") ou decimais ("0,00", usado por saÃ­das/NAC). Limitado
    // a no mÃ¡ximo os dois campos de delay (1Âº e 2Âº), pra nunca arriscar comer texto real.
  ...
  bands.forEach((band, i) => {
    // Monta o texto da linha (exceto o endereÃ§o) ordenando os itens da esquerda pra
    // direita (por x) â€” nÃ£o pela posiÃ§Ã£o estimada de cada palavra dentro de um item, jÃ¡
    // que o pdfjs pode juntar vÃ¡rias colunas num Ãºnico bloco de texto. Itens de uma mesma
  ...
  if (rows.length === 0) throw new Error('NÃ£o consegui extrair dispositivos desse relatÃ³rio LP1. Confira se o PDF nÃ£o estÃ¡ protegido ou escaneado como imagem.');
  return rows;
}

/* ---- Notifier (Honeywell): relatÃ³rios do VeriFire Tools em .xls/.xlsx/.csv ---- */
...
/** LÃª um arquivo .xls/.xlsx (via SheetJS) ou .csv e devolve um array de arrays (linhas x colunas). */
function readFileAsRows(file) {
  return new Promise((resolve, reject) => {
    const isExcel = /\.xlsx?$/i.test(file.name);
    const reader = new FileReader();
    reader.onerror = () => reject(new Error('NÃ£o foi possÃ­vel ler esse arquivo.'));
    if (isExcel) {
      reader.onload = () => {
        try {
          ...
          resolve(rows);
        } catch (e) { reject(new Error('NÃ£o consegui interpretar esse arquivo Excel.')); }
      };
...
/** Interpreta uma planilha exportada do VeriFire Tools (aba de MÃ³dulos ou Detectores). */
function parseNotifierSheet(rowsRaw) {
  const rows = (rowsRaw || []).filter((r) => (r || []).some((c) => String(c ?? '').trim() !== ''));
  if (rows.length < 4) throw new Error('NÃ£o reconheci esse arquivo como um export do VeriFire Tools da Notifier.');
  ...
  if (headerIdx === -1) throw new Error('NÃ£o encontrei a tabela de dispositivos nesse arquivo (cabeÃ§alho "Device ADDR" ausente).');
...
/** Converte uma planilha Notifier jÃ¡ interpretada para o formato genÃ©rico de linhas usado por buildImportEntities. */
function notifierSheetToGenericRows(parsedSheet, loopLabel) {
  return parsedSheet.rows.map((r) => {
    ...
    // endereÃ§os com INSTL=False sÃ£o posiÃ§Ãµes vazias no laÃ§o (sem dispositivo fÃ­sico) â€”
    // descartadas abaixo junto com linhas sem cÃ³digo de tipo.
  }).filter((r) => r.address && r.installed && r.type);
}

/* ---- Agrupamento genÃ©rico (comum a todas as marcas) ---- */
...
function buildImportEntities(rows, typeMap, brand, opts) {
  ...
  // Alguns mÃ³dulos MINI/DUAL MONITOR da Notifier (ex.: acionadores duplos) aparecem no
  // report como DOIS endereÃ§os inteiros separados com a mesma etiqueta de local, em vez
  // de um endereÃ§o com sub-endereÃ§o (ex.: 25 e 27, ambos "CABINE PRIMER"). PrÃ©-contamos
  // essas repetiÃ§Ãµes por laÃ§o para tratar isso como mÃ³dulo duplo (FDM-1) tambÃ©m.
  ...
  for (const r of rows) {
    ...
    if (!loopsByKey.has(loopKey)) {
      const loopNiceName = String(r.loop).replace(/^LP\s*(\d+)$/i, 'LaÃ§o $1');
    ...
    if (subs.length === 0) {
      ...
      for (const s of subs) {
        const label = s.label && s.label !== base.label
          ? (base.label ? `${base.label} â€” ${s.label}` : s.label)
          : (base.label || s.label);
...
const DEVICE_TYPES = [
  { value: 'fumaca', label: 'Detector de fumaÃ§a', icon: Cloud },
  { value: 'calor', label: 'Detector de calor', icon: Thermometer },
  { value: 'acionador', label: 'Acionador manual', icon: Hand },
  { value: 'saida', label: 'MÃ³dulo de saÃ­da', icon: LogOut },
  { value: 'entrada', label: 'MÃ³dulo de entrada', icon: LogIn },
  { value: 'rele', label: 'MÃ³dulo de relÃ©', icon: ToggleLeft },
];
...
const OPERATIONAL_STATUS_OPTIONS = [
  ...
  { value: 'nao_operante', label: 'NÃ£o operante' },
  { value: 'em_manutencao', label: 'Em manutenÃ§Ã£o' },
];
const APPEARANCE_OPTIONS = [
  { value: 'otimo', label: 'Ã“timo' },
  { value: 'bom', label: 'Bom' },
  { value: 'aceitavel', label: 'AceitÃ¡vel' },
...
const PUMP_TYPE_SUGGESTIONS = [
  'Bomba principal elÃ©trica', 'Bomba principal diesel', 'Bomba jockey',
  'Painel de controle da bomba', 'VÃ¡lvula de governo', 'ManÃ´metro',
  'Chave de fluxo', 'ReservatÃ³rio de Ã¡gua', 'Quadro de transferÃªncia automÃ¡tica',
];

const GAS_TYPE_SUGGESTIONS = [
  'GLP', 'GÃ¡s Natural (GN)', 'MonÃ³xido de carbono (CO)', 'Metano (CH4)', 'AmÃ´nia (NH3)',
];
...
const NAV_ITEMS = [
  ...
  { key: 'panels', label: 'PainÃ©is', icon: Cpu },
  { key: 'pumphouse', label: 'Casa de Bombas', icon: Droplet },
  { key: 'gas', label: 'Detectores de Gases', icon: Wind },
  { key: 'report', label: 'RelatÃ³rio', icon: ClipboardList },
  { key: 'history', label: 'HistÃ³rico', icon: Clock },
  { key: 'rvt', label: 'RVT', icon: Camera },
  { key: 'indicador', label: 'Indicador', icon: Activity },
  { key: 'graficos', label: 'GrÃ¡ficos', icon: BarChart3 },
  { key: 'settings', label: 'ConfiguraÃ§Ãµes', icon: Settings },
];
...
function computeStatus(nextDate) {
  ...
  if (diffDays < 0) {
    ...
    return { key: 'overdue', label: `Vencido hÃ¡ ${n} dia${n === 1 ? '' : 's'}`, color: 'var(--status-danger)' };
  }
...
/* Upload de mÃºltiplas fotos (com atalho de cÃ¢mera no celular), comprimindo cada uma. */
...
function allTrackableItems(data) {
  ...
  data.devices.forEach((d) => {
    ...
    items.push({
      ...
      meta: `${panel ? panel.name : 'â€”'} Â· ${loop ? loop.name : 'â€”'}${d.description ? ' Â· ' + d.description : ''}`,
  ...
  data.nacs.forEach((n) => {
    ...
    items.push({
      ...
      meta: `${panel ? panel.name : 'â€”'} Â· Circuito de saÃ­da${n.description ? ' Â· ' + n.description : ''}`,
  ...
  data.pumpDevices.forEach((p) => {
    items.push({
      ...
      meta: `Casa de Bombas${p.type ? ' Â· ' + p.type : ''}`,
  ...
  data.gasDetectors.forEach((g) => {
    items.push({
      ...
      meta: g.location || 'Detector de gÃ¡s fixo',
...
function getItemLabelAndContext(data, category, itemId) {
  if (category === 'devices') {
    ...
    return { label: `${typeLabel} Â· End. ${dItem.address}`, context: `${panel ? panel.name : 'â€”'} Â· ${loop ? loop.name : 'â€”'}` };
  }
  if (category === 'nacs') {
    const n = data.nacs.find((x) => x.id === itemId);
    if (!n) return { label: 'Circuito removido', context: '' };
    const panel = data.panels.find((p) => p.id === n.panelId);
    return { label: n.name, context: panel ? panel.name : 'â€”' };
  }
  if (category === 'pumpDevices') {
    ...
    return { label: p.name, context: `Casa de Bombas${p.type ? ' Â· ' + p.type : ''}` };
  }
  if (category === 'gasDetectors') {
    const g = data.gasDetectors.find((x) => x.id === itemId);
    if (!g) return { label: 'Detector removido', context: '' };
    return { label: g.name, context: g.location || 'Detector de gÃ¡s fixo' };
  }
  return { label: 'â€”', context: '' };
}
...
/* Sticky action row used by every form â€” keeps Save/Cancel reachable without
  ...
  return (
    <div className="rounded-lg p-3.5 flex flex-col gap-3" style={{ background: 'var(--surface)', border: selected ? '1px solid var(--accent)' : '1px solid var(--border)' }}>
      <div className="flex items-start gap-3">
        {selectable ? (
          <label className="pt-1.5 cursor-pointer" title={selected ? 'Remover da seleÃ§Ã£o' : 'Selecionar'}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs mt-1.5 mono" style={{ color: 'var(--text-secondary)' }}>
            Ãšltima manutenÃ§Ã£o: {formatDateBR(status && status.lastMaintenance)}
          </div>
          {!!indicadorCount && (
          ...
          {status && status.operationalStatus === 'nao_operante' && (
            <div className="text-xs mt-1 font-medium flex items-center gap-1" style={{ color: 'var(--status-danger)' }}>
              <AlertTriangle size={11} /> NÃ£o operante
            </div>
          )}
        </div>
      ...
      {!selectable && (
        <div className="flex items-center justify-end gap-1 pt-2" style={{ borderTop: '1px solid var(--border)' }}>
          {onInspect && <IconButton title="Registrar inspeÃ§Ã£o" onClick={onInspect}><ClipboardCheck size={15} /></IconButton>}
          {onMaintain && <IconButton title="Registrar manutenÃ§Ã£o" onClick={onMaintain}><Wrench size={15} /></IconButton>}
...
function PanelForm({ initial, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (v.name.trim()) onSubmit(v); }}>
      <Field label="IdentificaÃ§Ã£o do painel *"><input autoFocus className={inputCls} value={v.name}
        onChange={(e) => setV({ ...v, name: e.target.value })} placeholder="Ex.: Painel Central â€” TÃ©rreo" required /></Field>
      <Field label="LocalizaÃ§Ã£o"><input className={inputCls} value={v.location}
        onChange={(e) => setV({ ...v, location: e.target.value })} placeholder="Ex.: Sala de seguranÃ§a, Bloco A" /></Field>
      ...
      <Field label="Data de instalaÃ§Ã£o"><input type="date" className={inputCls} value={v.installDate}
        onChange={(e) => setV({ ...v, installDate: e.target.value })} /></Field>
      <Field label="ObservaÃ§Ãµes"><textarea rows={3} className={inputCls} value={v.notes}
...
function LoopForm({ initial, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (v.name.trim()) onSubmit(v); }}>
      <Field label="Nome / nÃºmero do laÃ§o *"><input autoFocus className={inputCls} value={v.name}
        onChange={(e) => setV({ ...v, name: e.target.value })} placeholder="Ex.: LaÃ§o 1" required /></Field>
      <FormActions>
        <Button variant="secondary" type="button" onClick={onCancel}>Cancelar</Button>
        <Button variant="primary" type="submit">Salvar laÃ§o</Button>
      </FormActions>
    </form>
  );
}
...
function NacForm({ initial, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (v.name.trim()) onSubmit(v); }}>
      <Field label="Nome / nÃºmero do circuito (NAC) *"><input autoFocus className={inputCls} value={v.name}
        onChange={(e) => setV({ ...v, name: e.target.value })} placeholder="Ex.: NAC 1 â€” Sirenes 1Âº pavimento" required /></Field>
      <Field label="DescriÃ§Ã£o"><input className={inputCls} value={v.description}
...
function DeviceForm({ initial, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (v.address.trim()) onSubmit(v); }}>
      <div className="grid grid-cols-2 gap-3">
        <Field label="EndereÃ§o *"><input autoFocus className={`${inputCls} mono`} value={v.address}
      ...
      <Field label="DescriÃ§Ã£o / localizaÃ§Ã£o"><input className={inputCls} value={v.description}
        onChange={(e) => setV({ ...v, description: e.target.value })} placeholder="Ex.: Corredor 2Âº andar, prÃ³x. sala 204" /></Field>
      <MaintenanceScheduleFields values={v} setValues={setV} />
      <FormActions>
        <Button variant="secondary" type="button" onClick={onCancel}>Cancelar</Button>
...
function PumpDeviceForm({ initial, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (v.name.trim()) onSubmit(v); }}>
      <Field label="IdentificaÃ§Ã£o *"><input autoFocus className={inputCls} value={v.name}
      ...
      <Field label="Tipo de equipamento">
        <input className={inputCls} list="pump-type-list" value={v.type}
          onChange={(e) => setV({ ...v, type: e.target.value })} placeholder="Ex.: Bomba principal elÃ©trica" />
      ...
      <Field label="ObservaÃ§Ãµes / localizaÃ§Ã£o"><input className={inputCls} value={v.description}
...
function GasDetectorForm({ initial, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (v.name.trim()) onSubmit(v); }}>
      <Field label="IdentificaÃ§Ã£o *"><input autoFocus className={inputCls} value={v.name}
        onChange={(e) => setV({ ...v, name: e.target.value })} placeholder="Ex.: Detector de gÃ¡s â€” Cozinha" required /></Field>
      <Field label="GÃ¡s monitorado">
      ...
      <Field label="LocalizaÃ§Ã£o"><input className={inputCls} value={v.location}
        onChange={(e) => setV({ ...v, location: e.target.value })} placeholder="Ex.: Casa de gÃ¡s, Ã¡rea externa" /></Field>
      <MaintenanceScheduleFields values={v} setValues={setV} />
      <FormActions>
        <Button variant="secondary" type="button" onClick={onCancel}>Cancelar</Button>
...
const RVT_CATEGORY_OPTIONS = [
  ...
  { value: 'nacs', label: 'Circuito de saÃ­da (NAC)' },
  { value: 'pumpDevices', label: 'Casa de Bombas' },
  { value: 'gasDetectors', label: 'Detector de GÃ¡s' },
  { value: 'outro', label: 'Outro / nÃ£o listado' },
];

/** FormulÃ¡rio de "adicionar item" do RVT â€” os campos de local (painel/laÃ§o/dispositivo)
    ficam travados nos cadastros jÃ¡ existentes, pra nÃ£o deixar o tÃ©cnico digitar errado. */
function RvtItemForm({ data, onAdd }) {
  ...
  function resolveSelected() {
    ...
    if (categoria === 'gasDetectors' && simpleId) {
      ...
      if (!it) return null;
      return { deviceId: it.id, categoria: 'gasDetectors', etiqueta: it.name, endereco: '', laco: '', painel: '', equipamento: 'Detector de GÃ¡s', area: '' };
    }
    ...
    <div className="rounded-lg p-3.5 flex flex-col gap-3" style={{ background: 'var(--surface-raised)', border: '1px dashed var(--border)' }}>
      ...
      {categoria === 'devices' && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          ...
          <Field label={`Dispositivo (endereÃ§o)${loopId && loopDevices.length > 8 ? ` â€” ${filteredLoopDevices.length} de ${loopDevices.length}` : ''}`}>
            {loopId && loopDevices.length > 8 && (
              <div className="relative mb-1.5">
                ...
                <input className={`${inputCls} pl-8`} placeholder="Buscar por endereÃ§o, tipo ou descriÃ§Ã£o..."
            ...
            <select className={inputCls} value={deviceId} onChange={(e) => setDeviceId(e.target.value)} disabled={!loopId}>
              ...
              {filteredLoopDevices.map((d) => (
                <option key={d.id} value={d.id}>{d.address} â€” {DEVICE_TYPE_MAP[d.type]?.label}{d.description ? ` (${d.description})` : ''}</option>
              ))}
            </select>
          </Field>
        </div>
      ...
      {categoria === 'nacs' && (
        <Field label="Circuito (NAC)">
          <select className={inputCls} value={simpleId} onChange={(e) => setSimpleId(e.target.value)}>
            ...
            {data.nacs.map((n) => {
              const panel = data.panels.find((p) => p.id === n.panelId);
              return <option key={n.id} value={n.id}>{n.name}{panel ? ` â€” ${panel.name}` : ''}</option>;
            })}
          </select>
        </Field>
      )}
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <Field label="Data de intervenÃ§Ã£o"><input type="date" className={inputCls} value={dataIntervencao} onChange={(e) => setDataIntervencao(e.target.value)} /></Field>
      </div>
      <Field label="ExplanaÃ§Ã£o"><textarea rows={2} className={inputCls} value={explanacao} onChange={(e) => setExplanacao(e.target.value)} /></Field>
      <Field label="SoluÃ§Ã£o"><textarea rows={2} className={inputCls} value={solucao} onChange={(e) => setSolucao(e.target.value)} /></Field>
      <MultiPhotoUpload photos={fotos} onChange={setFotos} />

      <Button variant="primary" type="button" onClick={handleAdd} disabled={!canAdd}><Plus size={16} /> Adicionar item ao relatÃ³rio</Button>
    </div>
  );
}
      ...
      <div className="print-area rounded-xl overflow-hidden flex flex-col" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <div className="rvt-brand-band">
          <div className="flex items-center gap-3 flex-wrap" style={{ position: 'relative', zIndex: 1 }}>
            <div className="rvt-wordmark">
              ...
              <div className="rvt-wordmark-text">
                ...
                <div className="sol">SoluÃ§Ãµes</div>
              </div>
            </div>
          ...
          <div className="text-right" style={{ position: 'relative', zIndex: 1 }}>
            <p className="font-display font-semibold text-base" style={{ color: '#fff', letterSpacing: '0.04em' }}>RELATÃ“RIO DE VISITA TÃ‰CNICA</p>
            <p className="text-xs mono" style={{ color: 'rgba(255,255,255,0.75)' }}>RVT Â· {formatDateBR(report.data)}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="rvt-summary-card rounded-lg p-3" style={{ background: 'var(--surface-raised)' }}>
            <RvtFieldLabel>TÃ©cnico responsÃ¡vel</RvtFieldLabel>
            <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{report.tecnico || 'â€”'}</p>
          </div>
          <div className="rvt-summary-card rounded-lg p-3" style={{ background: 'var(--surface-raised)' }}>
            ...
            <div key={it.id} className="rvt-item-card rounded-lg p-4" style={{ background: 'var(--surface-raised)', border: '1px solid var(--border)', breakInside: 'avoid' }}>
              ...
              {(it.endereco || it.laco || it.painel || it.equipamento) && (
                <div className="flex flex-wrap items-center gap-2 mb-3 text-xs" style={{ color: 'var(--text-secondary)' }}>
                  ...
                  {it.equipamento && <span>Â· {it.equipamento}</span>}
                </div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-2">
                <div>
                  ...
                  <p className="text-xs" style={{ color: 'var(--text-primary)' }}>{it.falha || 'â€”'}</p>
                </div>
                {it.dataIntervencao && (
                  <div>
                    <RvtFieldLabel>Data de intervenÃ§Ã£o</RvtFieldLabel>
              ...
              {it.explanacao && (
                <div className="mb-2">
                  <RvtFieldLabel>ExplanaÃ§Ã£o</RvtFieldLabel>
              ...
              {it.solucao && (
                <div className="mb-2 rounded-md p-2" style={{ background: 'rgba(79,138,109,0.08)' }}>
                  <RvtFieldLabel>SoluÃ§Ã£o aplicada</RvtFieldLabel>
              ...
              {it.fotos && it.fotos.length > 0 && (
                <div className="mt-3">
                  <RvtFieldLabel>Registro fotogrÃ¡fico</RvtFieldLabel>
        ...
        <div className="rvt-footer-band">
          ...
          <p className="text-[10px]" style={{ color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            Documento gerado pelo Centro de Controle de ManutenÃ§Ã£o â€” M.A.J Eletro EletrÃ´nica LTDA Â· CNPJ: 45.893.915/0001-01
          </p>
        </div>
        </div>
      </div>
...
function RvtView({ data, client, canEdit, onSaveReport, onDeleteReport }) {
    ...
    return (
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <h2 className="font-display text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>Novo RelatÃ³rio de Visita TÃ©cnica (RVT)</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <Field label="TÃ©cnico responsÃ¡vel"><input className={inputCls} value={tecnico} onChange={(e) => setTecnico(e.target.value)} placeholder="Nome do tÃ©cnico" /></Field>
        </div>
            ...
            {queue.map((it) => (
              <div key={it.id} className="rounded-lg p-3 flex items-start justify-between gap-2" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
                <div className="min-w-0">
                  <p className="text-sm font-medium truncate" style={{ color: 'var(--text-primary)' }}>{it.etiqueta}{it.endereco ? ` Â· END ${it.endereco}` : ''}</p>
                  <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>{it.falha} â€” {it.status}{it.fotos?.length ? ` Â· ${it.fotos.length} foto(s)` : ''}</p>
                </div>
        ...
        <Button variant="primary" onClick={handleSave} disabled={queue.length === 0}>
          <CheckCircle2 size={16} /> Salvar relatÃ³rio de visita ({queue.length} item{queue.length === 1 ? '' : 's'})
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h2 className="font-display text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>RVT â€” RelatÃ³rios de Visita TÃ©cnica</h2>
      ...
      {reports.length === 0 ? (
        <EmptyState icon={Camera} title="Nenhum RVT registrado ainda"
          description="Crie o primeiro relatÃ³rio de visita tÃ©cnica, selecionando os dispositivos direto dos painÃ©is jÃ¡ cadastrados."
        ...
        <div className="flex flex-col gap-2">
          {[...reports].sort((a, b) => b.data.localeCompare(a.data)).map((r) => (
            <div key={r.id} className="rounded-lg p-3.5 flex items-center justify-between gap-3 flex-wrap" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
              <div>
                <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{formatDateBR(r.data)}{r.tecnico ? ` â€” ${r.tecnico}` : ''}</p>
              ...
              <div className="flex gap-2">
                ...
                {canEdit && <IconButton title="Excluir relatÃ³rio" danger onClick={() => onDeleteReport(r)}><Trash2 size={15} /></IconButton>}
              </div>
            </div>
          ))}
        </div>
...
function IndicadorForm({ initial, areaSuggestions, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (v.etiqueta.trim() || v.falha.trim()) onSubmit(v); }}>
      <Field label="Etiqueta / LocalizaÃ§Ã£o *"><input autoFocus className={inputCls} value={v.etiqueta}
        onChange={(e) => setV({ ...v, etiqueta: e.target.value })} placeholder="Ex.: SECURITY OFFICE CORREDOR SL REUNIÃƒO" required /></Field>
      <div className="grid grid-cols-3 gap-3">
        <Field label="EndereÃ§o"><input className={`${inputCls} mono`} value={v.endereco}
          onChange={(e) => setV({ ...v, endereco: e.target.value })} placeholder="017" /></Field>
        <Field label="LaÃ§o"><input className={inputCls} value={v.laco}
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Equipamento"><input className={inputCls} value={v.equipamento}
          onChange={(e) => setV({ ...v, equipamento: e.target.value })} placeholder="Ex.: Sensor de fumaÃ§a" /></Field>
        <Field label="Ãrea">
      ...
      <Field label="ExplanaÃ§Ã£o"><textarea rows={2} className={inputCls} value={v.explanacao}
        onChange={(e) => setV({ ...v, explanacao: e.target.value })} /></Field>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Data diagnÃ³stico"><input type="date" className={inputCls} value={v.dataDiagnostico}
          onChange={(e) => setV({ ...v, dataDiagnostico: e.target.value })} /></Field>
        <Field label="Data soluÃ§Ã£o"><input type="date" className={inputCls} value={v.dataSolucao}
          onChange={(e) => setV({ ...v, dataSolucao: e.target.value })} /></Field>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <Field label="IntervenÃ§Ã£o 1"><input type="date" className={inputCls} value={v.dataIntervencao1}
          onChange={(e) => setV({ ...v, dataIntervencao1: e.target.value })} /></Field>
        <Field label="IntervenÃ§Ã£o 2"><input type="date" className={inputCls} value={v.dataIntervencao2}
          onChange={(e) => setV({ ...v, dataIntervencao2: e.target.value })} /></Field>
        <Field label="IntervenÃ§Ã£o 3"><input type="date" className={inputCls} value={v.dataIntervencao3}
          onChange={(e) => setV({ ...v, dataIntervencao3: e.target.value })} /></Field>
        <Field label="IntervenÃ§Ã£o 4"><input type="date" className={inputCls} value={v.dataIntervencao4}
          onChange={(e) => setV({ ...v, dataIntervencao4: e.target.value })} /></Field>
      </div>
      <Field label="SoluÃ§Ã£o"><textarea rows={2} className={inputCls} value={v.solucao}
...
function MaintenanceForm({ item, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(v); }}>
      <Field label="Tipo de manutenÃ§Ã£o">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Data da manutenÃ§Ã£o *"><input type="date" className={inputCls} value={v.date} required
          onChange={(e) => handleDateChange(e.target.value)} /></Field>
        <Field label="TÃ©cnico responsÃ¡vel"><input className={inputCls} value={v.technician}
          onChange={(e) => setV({ ...v, technician: e.target.value })} placeholder="Nome do tÃ©cnico" /></Field>
      </div>
      <Field label="ObservaÃ§Ãµes / serviÃ§o realizado"><textarea rows={3} className={inputCls} value={v.description}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="PrÃ³xima manutenÃ§Ã£o"><input type="date" className={inputCls} value={v.nextDate}
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(v); }}>
      <p className="text-xs mb-3" style={{ color: 'var(--text-secondary)' }}>
        Este registro serÃ¡ aplicado aos <strong>{count} dispositivos selecionados</strong>, com a mesma data, tÃ©cnico e observaÃ§Ãµes.
      </p>
      <Field label="Tipo de manutenÃ§Ã£o">
        <select className={inputCls} value={v.tipo} onChange={(e) => setV({ ...v, tipo: e.target.value })}>
          <option value="preventiva">Preventiva</option>
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(v); }}>
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="ComunicaÃ§Ã£o local">
        ...
        <Field label="ComunicaÃ§Ã£o em rede">
          <select className={inputCls} value={v.networkComm} onChange={(e) => setV({ ...v, networkComm: e.target.value })}>
            <option value="">NÃ£o avaliado</option>
            {COMM_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
        </Field>
        <Field label="Ãšltima inspeÃ§Ã£o"><input type="date" className={inputCls} value={v.lastInspection}
          onChange={(e) => setV({ ...v, lastInspection: e.target.value })} /></Field>
        <Field label="PrÃ³xima inspeÃ§Ã£o"><input type="date" className={inputCls} value={v.nextInspection}
          onChange={(e) => setV({ ...v, nextInspection: e.target.value })} /></Field>
      </div>
      <FormActions>
        <Button variant="secondary" type="button" onClick={onCancel}>Cancelar</Button>
        <Button variant="primary" type="submit"><CheckCircle2 size={15} /> Salvar inspeÃ§Ã£o</Button>
      </FormActions>
    </form>
  );
}
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (v.name.trim()) onSubmit(v); }} className={embedded ? 'max-w-lg' : ''}>
      <Field label="Nome do cliente *"><input autoFocus className={inputCls} value={v.name}
        onChange={(e) => setV({ ...v, name: e.target.value })} placeholder="Ex.: CondomÃ­nio Alfa" required /></Field>
      <Field label="EndereÃ§o"><input className={inputCls} value={v.address}
      ...
      <FormActions>
        ...
        <Button variant="primary" type="submit">{embedded ? 'Salvar alteraÃ§Ãµes' : 'Salvar cliente'}</Button>
      </FormActions>
    </form>
  );
}
...
function UserForm({ client, onSave, onRemove }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (v.fullName.trim() && v.username.trim() && v.password.trim()) onSave(v); }} className="max-w-lg flex flex-col">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="UsuÃ¡rio (login) *"><input className={inputCls} value={v.username}
      ...
      <p className="text-xs mb-3" style={{ color: 'var(--text-secondary)' }}>
        Este acesso fica salvo apenas neste navegador e serve para identificar quem usa o sistema deste cliente â€”
        nÃ£o Ã© uma autenticaÃ§Ã£o segura de servidor.
      </p>
      <div className="flex items-center justify-between gap-2">
        {existing ? <Button variant="danger" type="button" onClick={onRemove}><Trash2 size={14} /> Remover usuÃ¡rio</Button> : <span />}
        <Button variant="primary" type="submit">{existing ? 'Atualizar usuÃ¡rio' : 'Criar usuÃ¡rio'}</Button>
      </div>
    </form>
  );
}
...
function BrandingForm({ client, onSave }) {
  ...
  return (
    <div className="flex flex-col gap-5 max-w-lg">
      ...
      <div>
        <p className="text-sm font-medium mb-2" style={{ color: 'var(--text-primary)' }}>Cor da capa (cartÃ£o do cliente)</p>
        <div className="flex items-center gap-3">
  ...
  return (
    <div className="flex flex-col gap-4 max-w-2xl">
      <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
        Associe uma foto a cada modelo de equipamento. A foto vale para todos os dispositivos que usam esse modelo â€”
        por exemplo, uma Ãºnica foto para todos os detectores "ALO-V".
      </p>
...
function LoginGate({ client, onSuccess, onBack }) {
  ...
  function handleSubmit(e) {
    ...
    else setError('UsuÃ¡rio ou senha incorretos.');
  }
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'var(--bg)' }}>
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-sm rounded-2xl p-6 flex flex-col gap-1" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <div className="flex flex-col items-center gap-2 mb-3">
          ...
          <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>Entre com o usuÃ¡rio deste cliente</p>
        </div>
        <Field label="UsuÃ¡rio"><input className={inputCls} autoFocus value={username} onChange={(e) => setUsername(e.target.value)} required /></Field>
  ...
  return (
    <div className="rounded-xl overflow-hidden flex flex-col" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
      ...
      <div className="flex items-center justify-between gap-1 px-3.5 pb-3">
        {client.user ? <span className="mono-chip">usuÃ¡rio configurado</span> : <span />}
        {(onEdit || onDelete) && (
    ...
    <div className="min-h-screen relative overflow-hidden" style={{ background: 'var(--bg)' }}>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 flex flex-col gap-6 relative" style={{ zIndex: 1, minHeight: '100vh' }}>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3 text-center">
          ...
          <div>
            <h1 className="font-display font-semibold text-lg sm:text-2xl" style={{ color: 'var(--text-primary)' }}>Centro de Controle de ManutenÃ§Ã£o</h1>
        ...
        {clients.length === 0 ? (
          canManage ? (
            <EmptyState icon={Building2} title="Nenhum cliente cadastrado"
              description="Cadastre o primeiro cliente para comeÃ§ar a controlar as manutenÃ§Ãµes do sistema de PCI."
              actionLabel="Cadastrar cliente" onAction={() => setModal({ mode: 'create' })} />
          ) : (
            <EmptyState icon={Building2} title="Nenhum cliente vinculado Ã  sua conta"
              description="PeÃ§a ao administrador para vincular seu usuÃ¡rio a um cliente." />
          )
        ) : (
        ...
        <div className="mt-auto pt-8 text-center">
          <p className="text-[11px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
            Desenvolvido por M.A.J Eletro EletrÃ´nica LTDA â€” CNPJ: 45.893.915/0001-01<br />
            Sistema de uso exclusivo e restrito a usuÃ¡rios autorizados.
          </p>
        </div>
      </div>
      ...
      {confirm && canManage && (
        <ConfirmModal title="Excluir cliente"
          message={`Excluir "${confirm.name}"? Todos os painÃ©is, dispositivos e o histÃ³rico deste cliente serÃ£o removidos permanentemente.`}
...
function Workspace({ client, onUpdateClient, onSwitchClient }) {
  ...
  function importCsvEntities(entities) {
    ...
    setLastImport({
      ...
      summary: `${createdPanelIds.length} painel(Ã©is) novo(s), ${createdLoopIds.length} laÃ§o(s) novo(s) e ${createdDeviceIds.length} dispositivo(s)`,
    });
  }
  ...
  async function handleImportIndicador(file) {
    try {
      ...
      return { ok: false, error: err.message || 'NÃ£o foi possÃ­vel importar essa planilha.' };
    }
  }
  ...
  return (
    <div className="min-h-screen font-body" style={rootStyle}>
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs leading-tight" style={{ color: 'var(--text-secondary)' }}>Centro de Controle de ManutenÃ§Ã£o</p>
              </div>
            </div>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 pb-20">
        ...
        {view === 'panels' && (
          <PanelsList data={data} search={panelSearch} setSearch={setPanelSearch} canEdit={canEdit}
            ...
            onBulkDeletePanels={(ids) => setConfirmState({ title: 'Excluir painÃ©is selecionados', message: `Excluir ${ids.length} painel(Ã©is) selecionado(s)? Todos os laÃ§os, circuitos e dispositivos vinculados tambÃ©m serÃ£o removidos. Essa aÃ§Ã£o nÃ£o pode ser desfeita.`, onConfirm: () => deletePanelsBulk(ids) })} />
        )}
        ...
        {view === 'panelDetail' && panelId && (
          <PanelDetail
            ...
            onDeletePanel={(p) => setConfirmState({ title: 'Excluir painel', message: `Excluir "${p.name}"? Todos os laÃ§os, circuitos e dispositivos vinculados tambÃ©m serÃ£o removidos.`, onConfirm: () => deletePanelCascade(p.id) })}
            onCreateLoop={(pid) => {
              const n = data.loops.filter((l) => l.panelId === pid).length + 1;
              setModal({ type: 'loop', mode: 'create', initial: { name: `LaÃ§o ${n}` }, context: { panelId: pid } });
            }}
            onEditLoop={(l) => setModal({ type: 'loop', mode: 'edit', initial: l })}
            onDeleteLoop={(l) => setConfirmState({ title: 'Excluir laÃ§o', message: `Excluir "${l.name}"? Todos os dispositivos deste laÃ§o tambÃ©m serÃ£o removidos.`, onConfirm: () => deleteLoopCascade(l.id) })}
            onCreateNac={(pid) => {
            ...
            onDeleteDevice={(d) => setConfirmState({ title: 'Excluir dispositivo', message: `Excluir dispositivo endereÃ§o ${d.address}?`, onConfirm: () => deleteDevice(d.id) })}
            ...
            onBulkDeleteDevices={(ids) => setConfirmState({ title: 'Excluir dispositivos selecionados', message: `Excluir ${ids.length} dispositivo(s) selecionado(s)? Essa aÃ§Ã£o nÃ£o pode ser desfeita.`, onConfirm: () => deleteDevicesBulk(ids) })}
          />
        )}

        {view === 'pumphouse' && (
          <SimpleListView
            title="Casa de Bombas" description="Bombas, vÃ¡lvulas, painÃ©is de controle e demais equipamentos do sistema de bombeamento."
        ...
        {view === 'gas' && (
          <SimpleListView
            title="Detectores de Gases" description="Detectores fixos de gases inflamÃ¡veis ou tÃ³xicos instalados na edificaÃ§Ã£o."
            ...
            renderMeta={(g) => [g.gasType, g.location].filter(Boolean).join(' Â· ') || 'Detector de gÃ¡s fixo'} />
        )}
        ...
        {view === 'rvt' && (
          <RvtView data={data} client={client} canEdit={canEdit}
            ...
            onDeleteReport={(r) => setConfirmState({ title: 'Excluir relatÃ³rio de visita', message: `Excluir o RVT de ${formatDateBR(r.data)}? Os registros jÃ¡ criados no Indicador a partir dele nÃ£o serÃ£o apagados.`, onConfirm: () => deleteRvtReport(r.id) })} />
        )}
        ...
        {view === 'indicador' && (
          <IndicadorView data={data} canEdit={canEdit}
            ...
            onBulkDelete={(ids) => setConfirmState({ title: 'Excluir registros selecionados', message: `Excluir ${ids.length} registro(s) selecionado(s) do Indicador? Essa aÃ§Ã£o nÃ£o pode ser desfeita.`, onConfirm: () => deleteIndicadorBulk(ids) })}
            onDeleteByStatus={(status, count) => setConfirmState({ title: 'Excluir por status', message: `Excluir ${count} registro(s) com status "${status}"? Essa aÃ§Ã£o nÃ£o pode ser desfeita.`, onConfirm: () => deleteIndicadorByStatus(status) })}
            onDeleteAll={(count) => setConfirmState({ title: 'Excluir todos os registros', message: `Excluir todos os ${count} registro(s) do Indicador deste cliente? Essa aÃ§Ã£o nÃ£o pode ser desfeita.`, onConfirm: () => deleteIndicadorAll() })} />
        )}
      ...
      {modal?.type === 'loop' && (
        <Modal title={modal.mode === 'create' ? 'Novo laÃ§o' : 'Editar laÃ§o'} onClose={closeModal}>
          <LoopForm initial={modal.initial} onSubmit={submitLoop} onCancel={closeModal} />
        </Modal>
      )}
      {modal?.type === 'nac' && (
        <Modal title={modal.mode === 'create' ? 'Novo circuito de saÃ­da (NAC)' : 'Editar circuito (NAC)'} onClose={closeModal} wide>
      ...
      {modal?.type === 'pump' && (
        <Modal title={modal.mode === 'create' ? 'Novo dispositivo â€” Casa de Bombas' : 'Editar dispositivo'} onClose={closeModal} wide>
          <PumpDeviceForm initial={modal.initial} onSubmit={submitPumpDevice} onCancel={closeModal} />
        </Modal>
      )}
      {modal?.type === 'gas' && (
        <Modal title={modal.mode === 'create' ? 'Novo detector de gÃ¡s' : 'Editar detector'} onClose={closeModal} wide>
      ...
      {modal?.type === 'maintenance' && (
        <Modal title={`Registrar manutenÃ§Ã£o â€” ${modal.context.label}`} onClose={closeModal} wide>
          <MaintenanceForm item={modal.item} onSubmit={submitMaintenance} onCancel={closeModal} />
        </Modal>
      )}
      {modal?.type === 'bulkMaintenance' && (
        <Modal title={`Registrar manutenÃ§Ã£o em lote (${modal.context.ids.length} dispositivos)`} onClose={closeModal} wide>
      ...
      {modal?.type === 'inspection' && (
        <Modal title={`Registrar inspeÃ§Ã£o â€” ${modal.context.label}`} onClose={closeModal} wide>
...
/** GrÃ¡fico de barras horizontais simples, em SVG/HTML puro (sem dependÃªncias externas). */
...
/** GrÃ¡fico de pizza/rosca simples, em SVG puro, com legenda ao lado. */
...
/** Baixa uma lista de pares [rÃ³tulo, valor] como CSV (separador ; para abrir bem no Excel BR). */
...
/** Conta itens com campo `date` (YYYY-MM-DD) nos Ãºltimos N meses, agrupados por mÃªs. */
...
function Dashboard({ data, counts, attentionItems, canEdit, onMaintain, onInspect, onGoPanels }) {
  ...
  if (data.panels.length === 0 && data.pumpDevices.length === 0 && data.gasDetectors.length === 0) {
    return (
      <EmptyState icon={Cpu} title="Nenhum equipamento cadastrado ainda"
        description="Comece cadastrando o primeiro painel de detecÃ§Ã£o e alarme de incÃªndio. Depois vocÃª poderÃ¡ adicionar laÃ§os, circuitos e dispositivos endereÃ§Ã¡veis."
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        <h3 className="font-medium text-sm mb-3" style={{ color: 'var(--text-primary)' }}>Itens que precisam de atenÃ§Ã£o</h3>
        {attentionItems.length === 0 ? (
          <div className="rounded-xl p-6 flex items-center gap-3" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            ...
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Nenhum item vencido ou prÃ³ximo do vencimento. Tudo em dia.</p>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 gap-3">
...
function PanelsList({ data, search, setSearch, canEdit, onOpenPanel, onCreate, onBulkDeletePanels }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <h2 className="font-display text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>PainÃ©is</h2>
        {canEdit && (
          <div className="flex gap-2">
            {data.panels.length > 0 && (
              selectMode ? (
                <Button variant="secondary" onClick={exitSelectMode}><X size={15} /> Cancelar seleÃ§Ã£o</Button>
              ) : (
                <Button variant="secondary" onClick={() => setSelectMode(true)}><ClipboardList size={15} /> Selecionar mÃºltiplos</Button>
              )
            )}
      ...
      {selectMode && (
        <div className="rounded-lg p-3 flex items-center justify-between gap-3 flex-wrap" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
          <p className="text-sm" style={{ color: 'var(--text-primary)' }}>
            {selectedIds.length === 0 ? 'Marque os painÃ©is que quer excluir de uma vez.' : `${selectedIds.length} painel(Ã©is) selecionado(s)`}
          </p>
      ...
      {data.panels.length > 0 && (
        <div className="relative">
          ...
          <input className={`${inputCls} pl-9`} placeholder="Buscar painel por nome ou localizaÃ§Ã£o..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      )}

      {data.panels.length === 0 ? (
        <EmptyState icon={Cpu} title="Nenhum painel cadastrado"
          description="Cadastre o painel de detecÃ§Ã£o e alarme de incÃªndio. Em seguida vocÃª poderÃ¡ adicionar laÃ§os (loops), circuitos de saÃ­da (NACs) e os dispositivos endereÃ§Ã¡veis."
            ...
            const content = (
              <>
                ...
                <div className="flex gap-3 text-xs mono mt-1" style={{ color: 'var(--text-secondary)' }}>
                  <span>{loops.length} laÃ§o{loops.length === 1 ? '' : 's'}</span>
  ...
  return (
    <div className="flex flex-col gap-5">
      <button onClick={onBack} className="flex items-center gap-1.5 text-sm w-fit" style={{ color: 'var(--text-secondary)' }}>
        <ArrowLeft size={15} /> PainÃ©is
      </button>

      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h2 className="font-display text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>{panel.name}</h2>
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>{[panel.location, panel.model].filter(Boolean).join(' Â· ') || 'Sem informaÃ§Ãµes adicionais'}</p>
        </div>
        {canEdit && (
          <div className="flex gap-1">
      ...
      <div className="flex gap-1 border-b" style={{ borderColor: 'var(--border)' }}>
        <button className="nav-tab" data-active={tab === 'loops'} onClick={() => setTab('loops')}>LaÃ§os ({loops.length})</button>
        <button className="nav-tab" data-active={tab === 'nacs'} onClick={() => setTab('nacs')}>Circuitos de saÃ­da â€” NAC ({nacs.length})</button>
      </div>
        ...
        <div className="flex flex-col gap-3">
          {canEdit && (
            <div className="flex justify-end gap-2">
              ...
              <Button variant="primary" onClick={() => onCreateLoop(panelId)}><Plus size={15} /> Novo laÃ§o</Button>
            </div>
          )}
          {selectMode && (
            <div className="rounded-lg p-3 flex items-center justify-between gap-3 flex-wrap" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
              ...
              {selectedIds.length > 0 && (
                <div className="flex gap-2">
                  ...
                  <Button variant="primary" onClick={() => { onBulkMaintainDevices(selectedIds); exitSelectMode(); }}><Wrench size={15} /> Registrar manutenÃ§Ã£o</Button>
                </div>
              )}
            </div>
          )}
          {loops.length === 0 ? (
            <EmptyState icon={Cpu} title="Nenhum laÃ§o cadastrado" description="Adicione um laÃ§o para comeÃ§ar a cadastrar detectores, acionadores e mÃ³dulos endereÃ§Ã¡veis."
              actionLabel={canEdit ? 'Adicionar laÃ§o' : undefined} onAction={canEdit ? () => onCreateLoop(panelId) : undefined} />
          ) : (
            <>
              {loops.some((l) => data.devices.some((d) => d.loopId === l.id)) && (
                <div className="relative">
                  <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-secondary)' }} />
                  <input className={`${inputCls} pl-9`} placeholder="Buscar dispositivo por endereÃ§o, modelo ou local..."
                    ...
                    <div className="flex items-center gap-2 p-3.5">
                      ...
                      {canEdit && (
                        <>
                          ...
                          <IconButton title="Editar laÃ§o" onClick={() => onEditLoop(loop)}><Pencil size={15} /></IconButton>
                          <IconButton title="Excluir laÃ§o" danger onClick={() => onDeleteLoop(loop)}><Trash2 size={15} /></IconButton>
                        </>
                      )}
                    </div>
                    {expanded && (
                      <div className="px-3.5 pb-3.5 flex flex-col gap-2">
                        {devices.length === 0 ? (
                          <p className="text-xs py-3 text-center" style={{ color: 'var(--text-secondary)' }}>
                            {q ? 'Nenhum dispositivo corresponde Ã  busca.' : 'Nenhum dispositivo neste laÃ§o ainda.'}
                          </p>
                        ) : devices.map((d) => (
                          <TrackableCard key={d.id} icon={DEVICE_TYPE_MAP[d.type]?.icon} photo={photoForModelo(data, d.modelo)} address={d.address}
                            title={(DEVICE_TYPE_MAP[d.type]?.label || 'Dispositivo') + (d.modelo ? ` Â· ${d.modelo}` : '')} meta={d.description}
      ...
      {tab === 'nacs' && (
        <div className="flex flex-col gap-3">
          ...
          {nacs.length === 0 ? (
            <EmptyState icon={Bell} title="Nenhum circuito de saÃ­da cadastrado" description="Cadastre os circuitos (NACs) que alimentam sirenes, strobos e demais dispositivos de notificaÃ§Ã£o."
...
function SimpleListView({ title, description, icon, data, category, canEdit, onCreate, onEdit, onDelete, onMaintain, onInspect, renderMeta }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {list.length === 0 ? (
        <EmptyState icon={icon} title="Nenhum item cadastrado" description="Adicione o primeiro equipamento para comeÃ§ar a acompanhar as manutenÃ§Ãµes."
...
function HistoryView({ data, filter, setFilter }) {
  const filters = [
    ...
    { value: 'pumpDevices', label: 'Casa de Bombas' },
    { value: 'gasDetectors', label: 'Detectores de gÃ¡s' },
  ];
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h2 className="font-display text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>HistÃ³rico de manutenÃ§Ãµes</h2>
        <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Registro de todas as manutenÃ§Ãµes realizadas, da mais recente para a mais antiga.</p>
      </div>
      ...
      {entries.length === 0 ? (
        <EmptyState icon={Inbox} title="Nenhum registro encontrado" description="As manutenÃ§Ãµes registradas em dispositivos, circuitos e demais equipamentos aparecerÃ£o aqui." />
      ) : (
        <div className="flex flex-col gap-2">
          {entries.map((entry) => {
              ...
              <div key={entry.id} className="rounded-lg p-3.5 flex flex-col gap-1" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
                ...
                {(entry.technician || entry.description) && (
                  <div className="text-xs mt-1" style={{ color: 'var(--text-secondary)' }}>
                    {entry.technician && <span>TÃ©cnico: {entry.technician}. </span>}
                    {entry.description}
                  </div>
                )}
                {entry.nextDate && <div className="text-xs mono mt-1" style={{ color: 'var(--text-secondary)' }}>PrÃ³xima manutenÃ§Ã£o: {formatDateBR(entry.nextDate)}</div>}
              </div>
            );
          })}
        </div>
...
function IndicadorView({ data, canEdit, onCreate, onEdit, onDelete, onImportFile, onLinkDevices, onBulkDelete, onDeleteByStatus, onDeleteAll }) {
  ...
  function deviceContext(r) {
    ...
    if (cat === 'devices') {
      ...
      const panel = loop && data.panels.find((p) => p.id === loop.panelId);
      return {
        title: (DEVICE_TYPE_MAP[device.type]?.label || 'Dispositivo') + (device.modelo ? ` Â· ${device.modelo}` : ''),
        address: device.address, sub: [loop?.name, panel?.name].filter(Boolean).join(' Â· '),
      };
    }
    if (cat === 'nacs') {
      ...
      if (!nac) return null;
      const panel = data.panels.find((p) => p.id === nac.panelId);
      return { title: nac.name, address: null, sub: ['Circuito NAC', panel?.name].filter(Boolean).join(' Â· ') };
    }
    if (cat === 'pumpDevices') {
    ...
    if (cat === 'gasDetectors') {
      ...
      if (!it) return null;
      return { title: it.name, address: null, sub: 'Detector de GÃ¡s' };
    }
    return null;
  }
  ...
  function handleLink() {
    ...
    setLinkMsg(`${res.matched} de ${res.total} registro(s) vinculado(s) a um dispositivo importado. ${res.unmatched} sem correspondÃªncia (painel/laÃ§o/endereÃ§o nÃ£o encontrados entre os dispositivos jÃ¡ importados).`);
  }
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>HistÃ³rico de diagnÃ³stico e falhas identificadas no sistema.</p>
        </div>
        {canEdit && (
          <div className="flex flex-wrap gap-2">
            <label className="btn btn-secondary cursor-pointer" style={importing ? { opacity: 0.6, pointerEvents: 'none' } : undefined}>
              <Upload size={15} /> {importing ? 'Importandoâ€¦' : 'Importar planilha (.xlsx)'}
      ...
      {list.length > 0 && (
        <>
          <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
            {linkedCount} de {list.length} registro(s) jÃ¡ vinculado(s) a um dispositivo do painel.
          </p>
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative flex-1">
              <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-secondary)' }} />
              <input className={`${inputCls} pl-9`} placeholder="Buscar por etiqueta, endereÃ§o, falha..." value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
            ...
            <select className={inputCls} style={{ maxWidth: '220px' }} value={linkFilter} onChange={(e) => setLinkFilter(e.target.value)}>
              <option value="all">Vinculados e nÃ£o vinculados</option>
              <option value="linked">SÃ³ vinculados a dispositivo</option>
              <option value="unlinked">SÃ³ sem vÃ­nculo</option>
            </select>
          </div>
        </>
      )}
      ...
      {list.length === 0 ? (
        <EmptyState icon={Activity} title="Nenhum registro no indicador"
          description="Importe a planilha de histÃ³rico de diagnÃ³stico ou cadastre o primeiro registro manualmente."
              ...
              <div key={r.id} className="rounded-lg p-3.5 flex flex-col gap-2" style={{ background: 'var(--surface)', border: isSelected ? '1px solid var(--accent)' : ctx ? '1px solid var(--accent)' : '1px solid var(--border)' }}>
                <div className="flex items-start justify-between gap-2 flex-wrap">
                  <div className="flex items-start gap-2 min-w-0">
                    {selectMode && (
                      <label className="pt-1 cursor-pointer flex-shrink-0" title={isSelected ? 'Remover da seleÃ§Ã£o' : 'Selecionar'}>
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-1 text-xs" style={{ color: 'var(--text-secondary)' }}>
                        {ctx ? (
                          <>
                            ...
                            {ctx.sub && <span>Â· {ctx.sub}</span>}
                          </>
                        ) : (
                          <>
                            {r.endereco && <span className="mono-chip">END {r.endereco}</span>}
                            {r.laco && <span className="mono-chip">LaÃ§o {r.laco}</span>}
                            {r.painel && <span className="mono-chip">Painel {r.painel}</span>}
                            {r.area && <span>{r.area}</span>}
                          </>
                        )}
                        {r.equipamento && <span>Â· {r.equipamento}</span>}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {r.explanacao && <p className="text-xs" style={{ color: 'var(--text-secondary)' }}><strong>ExplanaÃ§Ã£o:</strong> {r.explanacao}</p>}
                {r.solucao && <p className="text-xs" style={{ color: 'var(--status-ok)' }}><strong>SoluÃ§Ã£o:</strong> {r.solucao}</p>}
                ...
                {(r.dataDiagnostico || r.dataSolucao) && (
                  <div className="flex flex-wrap gap-3 text-xs mono" style={{ color: 'var(--text-secondary)' }}>
                    {r.dataDiagnostico && <span>DiagnÃ³stico: {formatDateBR(r.dataDiagnostico)}</span>}
                    {r.dataSolucao && <span>SoluÃ§Ã£o: {formatDateBR(r.dataSolucao)}</span>}
                  </div>
                )}
                {canEdit && !selectMode && (
                  <div className="flex items-center justify-end gap-1 pt-2" style={{ borderTop: '1px solid var(--border)' }}>
...
function ChartsView({ data }) {
  ...
  const areaData = sortDesc(countBy(indicador, (r) => r.area, 'Sem Ã¡rea')).slice(0, 10);
  const falhaData = sortDesc(countBy(indicador, (r) => r.falha, 'Sem falha')).slice(0, 10);

  const CATEGORY_LABELS = { devices: 'Dispositivos', nacs: 'Circuitos (NAC)', pumpDevices: 'Casa de Bombas', gasDetectors: 'Detectores de gÃ¡s' };
  const tipoData = [
    ...
    { label: 'InspeÃ§Ã£o', value: inspectionLog.length, color: 'var(--accent)' },
  ].filter((d) => d.value > 0);
  ...
  function exportIndicadorCsv() {
    exportChartCsv('indicador-resumo.csv', ['Categoria', 'RÃ³tulo', 'Quantidade'], [
      ...statusData.map((d) => ['Status', d.label, d.value]),
      ...areaData.map((d) => ['Ãrea', d.label, d.value]),
      ...falhaData.map((d) => ['Falha', d.label, d.value]),
    ]);
  }
  function exportManutencaoCsv() {
    exportChartCsv('manutencoes-resumo.csv', ['Categoria', 'RÃ³tulo', 'Quantidade'], [
      ...tipoData.map((d) => ['Tipo', d.label, d.value]),
      ...categoryData.map((d) => ['Categoria do item', d.label, d.value]),
      ...monthData.map((d) => ['MÃªs', d.label, d.value]),
    ]);
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-3 flex-wrap no-print">
        <div>
          <h2 className="font-display text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>GrÃ¡ficos</h2>
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>VisÃ£o geral do Indicador e das manutenÃ§Ãµes realizadas nos painÃ©is.</p>
        </div>
        <Button variant="primary" onClick={() => window.print()}><Printer size={16} /> Imprimir / Salvar PDF</Button>
      </div>
      ...
      <div className="print-area flex flex-col gap-8 rounded-xl p-4 sm:p-5" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <div>
              <h3 className="font-display text-base font-semibold" style={{ color: 'var(--text-primary)' }}>Indicador â€” diagnÃ³stico e falhas</h3>
          ...
          {!hasIndicador ? (
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <ChartCard title="Registros por Ã¡rea" subtitle="Onde as falhas mais aparecem"><SimpleBarChart data={areaData} /></ChartCard>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <div>
              <h3 className="font-display text-base font-semibold" style={{ color: 'var(--text-primary)' }}>ManutenÃ§Ãµes realizadas</h3>
          ...
          {!hasManutencao ? (
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Nenhuma manutenÃ§Ã£o ou inspeÃ§Ã£o registrada ainda.</p>
          ) : (
            <div className="grid sm:grid-cols-2 gap-4">
              <ChartCard title="Por tipo" subtitle="Preventiva, corretiva e inspeÃ§Ã£o"><SimplePieChart data={tipoData} /></ChartCard>
              <ChartCard title="Por categoria de equipamento"><SimpleBarChart data={categoryData} /></ChartCard>
              <ChartCard title="Ãšltimos 12 meses" subtitle="Volume por mÃªs"><SimpleBarChart data={monthData} emptyLabel="Sem manutenÃ§Ãµes nos Ãºltimos 12 meses." /></ChartCard>
            </div>
          )}
        </div>
      </div>
...
function ReportView({ data, client, filters, setFilters }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-3 flex-wrap no-print">
        <div>
          <h2 className="font-display text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>RelatÃ³rio de ManutenÃ§Ãµes Preventivas</h2>
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Status de verificaÃ§Ã£o de todos os dispositivos, circuitos e equipamentos.</p>
        </div>
        <Button variant="primary" onClick={() => window.print()}><Printer size={16} /> Imprimir / Salvar PDF</Button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 no-print">
        ...
        <StatCard label="NÃ£o operantes" value={summary.naoOperante} color="var(--status-danger)" icon={AlertTriangle} />
      ...
      <div className="print-area rounded-xl p-4 sm:p-5" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <div className="flex items-center justify-between gap-3 mb-4 pb-4 flex-wrap" style={{ borderBottom: '1px solid var(--border)' }}>
          ...
          <div className="text-right">
            <p className="font-medium text-sm" style={{ color: 'var(--text-primary)' }}>RelatÃ³rio de ManutenÃ§Ãµes Preventivas</p>
            ...
            <table className="w-full text-xs" style={{ borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  {['IdentificaÃ§Ã£o', 'LocalizaÃ§Ã£o', 'Status', 'AparÃªncia', 'Com. local', 'Com. rede', 'Ãšltima insp.', 'PrÃ³xima insp.'].map((h) => (
                  ...
                  <tr key={`${it.category}-${it.id}`} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td className="py-2 pr-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span style={{ color: 'var(--text-primary)' }}>{it.title}{it.modelo ? ` Â· ${it.modelo}` : ''}</span>
                      </div>
                    </td>
...
function SettingsView({ client, data, tab, setTab, onUpdateClient, onSaveModelPhoto, onRemoveModelPhoto, onImportCsv, lastImport, onUndoImport }) {
  const TABS = [
    ...
    { key: 'usuario', label: 'UsuÃ¡rio', icon: UserCog },
  ...
  return (
    <div className="flex flex-col gap-4">
      <h2 className="font-display text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>ConfiguraÃ§Ãµes</h2>
...
/* ------------------------------------------------------------------ */
/* ImportaÃ§Ã£o de base de dados (CSV de painel)                        */
/* ------------------------------------------------------------------ */

/* Exporta a base atual de dispositivos (todos os painÃ©is/laÃ§os do cliente) em CSV,
   pronta para abrir no Excel (separador ; e BOM para acentuaÃ§Ã£o). */
/* ------------------------------------------------------------------ */
/* Indicador: histÃ³rico de diagnÃ³stico e falhas (planilha "Indicador SDAI") */
/* ------------------------------------------------------------------ */
...
/** LÃª a planilha "Indicador" (histÃ³rico de diagnÃ³stico/falhas) e devolve os registros
    jÃ¡ no formato usado pelo app, tolerando pequenas variaÃ§Ãµes de cabeÃ§alho (acentos,
    espaÃ§os extras) jÃ¡ que casa por conteÃºdo normalizado, nÃ£o pelo texto exato. */
async function parseIndicadorXlsx(file) {
  ...
  if (records.length === 0) {
    throw new Error('Nenhum registro foi encontrado nessa planilha. Verifique se Ã© o arquivo do Indicador.');
  }
  return records;
}
...
/* Vincula registros do Indicador aos dispositivos reais do painel, cruzando pela coluna
   ...
   Explorer), pelo laÃ§o e pelo endereÃ§o. Mapeamento tirado diretamente do report LP1 da
   Nissan (Panel X / Network Address N) â€” especÃ­fico deste sistema; painÃ©is fora dessa
   lista (ex.: 7 = AUTOLEARN/PWT, ou "Notifier") nÃ£o tÃªm correspondÃªncia aqui e ficam sem
   vÃ­nculo, jÃ¡ que pertencem a outro relatÃ³rio/sistema. */
...
function exportDevicesCsv(data) {
  const header = ['Painel', 'Modelo do Painel', 'LaÃ§o', 'EndereÃ§o', 'Categoria', 'Modelo do Dispositivo', 'DescriÃ§Ã£o', 'Ãšltima ManutenÃ§Ã£o', 'PrÃ³xima ManutenÃ§Ã£o'];
...
function ImportCsvView({ onImport, data, lastImport, onUndoImport }) {
  ...
  function recomputeFromRows(nextRows, nextTypeMap) {
    try {
      ...
      setError(err.message || 'NÃ£o foi possÃ­vel processar esses dados.');
      setEntities(null);
    }
  }
  ...
  function handleHochikiFile(e) {
    ...
    reader.onload = () => {
      try {
        ...
        setError(err.message || 'NÃ£o foi possÃ­vel ler esse arquivo.');
  ...
  async function handleHochikiLp1File(e) {
    ...
    try {
      ...
      setError(err.message || 'NÃ£o foi possÃ­vel ler esse arquivo PDF.');
      setRows(null); setEntities(null);
    }
  }
  ...
  function handleTargetPanelChange(id) {
    setTargetPanelId(id);
    if (rows) {
      // recompute usando o novo painel selecionado diretamente (o state ainda nÃ£o atualizou no closure)
      try {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {lastImport && (
        <div className="rounded-lg p-3.5 flex items-center justify-between gap-3 flex-wrap" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
          <div>
            <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>Ãšltima importaÃ§Ã£o</p>
          ...
          {confirmUndo ? (
            ...
            <Button variant="secondary" onClick={() => setConfirmUndo(true)}><Trash2 size={15} /> Desfazer Ãºltima importaÃ§Ã£o</Button>
          )}
        </div>
      )}
      ...
      <div className="rounded-lg p-3.5 flex flex-col gap-3" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        ...
        <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
          Envie o relatÃ³rio exportado do painel. O sistema identifica automaticamente painÃ©is, laÃ§os e dispositivos, de acordo com a marca e o modelo selecionados.
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label="Painel de destino">
            <select value={targetMode} onChange={(e) => handleTargetModeChange(e.target.value)}
              ...
              <option value="existing">Usar painel jÃ¡ cadastrado</option>
            </select>
          </Field>
          {targetMode === 'existing' && (
        ...
        {currentModel.loops > 1 && (
          <Field label="LaÃ§o correspondente a este arquivo" hint="A Notifier nÃ£o indica o laÃ§o no relatÃ³rio â€” informe a qual laÃ§o esses dispositivos pertencem.">
            <select value={loopChoice} onChange={(e) => handleLoopChoiceChange(Number(e.target.value))}
              ...
              {Array.from({ length: currentModel.loops }, (_, i) => i + 1).map((n) => (
                <option key={n} value={n}>LaÃ§o {n}</option>
              ))}
            </select>
          </Field>
        )}
        ...
        {brand === 'hochiki' ? (
          <div className="flex flex-col gap-2">
            <Field label="Formato do relatÃ³rio" hint="O Loop Explorer 2 exporta .csv; o Loop Explorer 1 (mais antigo) exporta .pdf.">
          ...
          <div className="flex flex-col gap-2">
            <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
              A Notifier separa detectores e mÃ³dulos em relatÃ³rios distintos no VeriFire Tools. Envie um ou os dois, conforme disponÃ­vel (.xls, .xlsx ou .csv).
            </p>
            <div className="flex flex-wrap gap-2">
              <label className="inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm w-fit cursor-pointer"
                style={{ background: 'var(--surface-raised)', border: '1px solid var(--border)', color: 'var(--text-primary)' }}>
                <FileSpreadsheet size={15} /> RelatÃ³rio de mÃ³dulos
                <input type="file" accept=".xls,.xlsx,.csv" onChange={(e) => handleNotifierFile('modules', e)} className="hidden" />
              </label>
              <label className="inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm w-fit cursor-pointer"
                style={{ background: 'var(--surface-raised)', border: '1px solid var(--border)', color: 'var(--text-primary)' }}>
                <FileSpreadsheet size={15} /> RelatÃ³rio de detectores
        ...
        {(fileNames.hochiki || fileNames.hochikiLp1 || fileNames.modules || fileNames.detectors) && (
          <div className="flex flex-col gap-0.5">
            ...
            {fileNames.modules && <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>MÃ³dulos: {fileNames.modules}</p>}
        ...
        {done && <p className="text-xs" style={{ color: 'var(--status-ok)' }}>ImportaÃ§Ã£o concluÃ­da! Confira em "PainÃ©is".</p>}
      </div>
        ...
        <>
          <div className="rounded-lg p-3.5 flex flex-col gap-2" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            ...
            <p className="text-xs mb-1" style={{ color: 'var(--text-secondary)' }}>
              Identifiquei estes cÃ³digos no arquivo. Ajuste se alguma categoria nÃ£o estiver certa.
            </p>
            {Object.keys(entities.typeCounts).map((code) => (
              <div key={code} className="flex items-center justify-between gap-2 rounded-md p-2" style={{ background: 'var(--surface-raised)' }}>
                <div className="min-w-0">
                  ...
                  <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>{entities.typeCounts[code]} ocorrÃªncia(s)</p>
                </div>
          ...
          <div className="rounded-lg p-3.5 flex flex-col gap-2" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            ...
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
              {entities.panels.length} painel(Ã©is) novo(s) Â· {entities.loops.length} laÃ§o(s) Â· {entities.devices.length} dispositivo(s)
            </p>
            {targetMode === 'existing' && existingPanel && (
              <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                Os dispositivos serÃ£o adicionados ao painel <strong>{existingPanel.name}</strong>. LaÃ§os com nome igual a um jÃ¡ existente serÃ£o reaproveitados; os demais serÃ£o criados.
              </p>
            )}
            <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
              Isso vai <strong>adicionar</strong> novos registros Ã  lista atual (nÃ£o substitui o que jÃ¡ existe). Importar o mesmo arquivo duas vezes cria duplicados.
            </p>
            <Button variant="primary" onClick={handleConfirm} disabled={targetMode === 'existing' && !existingPanel}>
              <Upload size={15} /> Confirmar importaÃ§Ã£o
            </Button>
          </div>
        </>
      )}
...
function MembersManager({ clientId }) {
  ...
  async function loadMembers() {
    ...
    if (err) { setError('NÃ£o foi possÃ­vel carregar os usuÃ¡rios deste cliente.'); return; }
    if (!rows || rows.length === 0) { setMembers([]); return; }
    const ids = rows.map((r) => r.user_id);
    const { data: profs } = await supabase.from('profiles').select('id, email').in('id', ids);
    const emailById = Object.fromEntries((profs || []).map((p) => [p.id, p.email]));
    setMembers(rows.map((r) => ({ ...r, email: emailById[r.user_id] || '(email nÃ£o encontrado)' })));
  }
  ...
  async function addMember(e) {
    ...
    try {
      ...
      if (!prof) {
        setError('Nenhum usuÃ¡rio encontrado com esse email. PeÃ§a para essa pessoa criar uma conta na tela de login do app primeiro, depois tente novamente.');
        return;
      }
      ...
      setInfo('UsuÃ¡rio vinculado com sucesso.');
      setEmail('');
      await loadMembers();
    } catch (err) {
      setError(err.message || 'NÃ£o foi possÃ­vel vincular esse usuÃ¡rio.');
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="rounded-lg p-3.5 flex flex-col gap-3" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>Vincular usuÃ¡rio a este cliente</p>
        <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
          A pessoa precisa primeiro ter criado uma conta na tela de login do app (com o email abaixo). Depois, vincule aqui e escolha o nÃ­vel de acesso.
        </p>
        <form onSubmit={addMember} className="flex flex-col sm:flex-row gap-2">
          ...
          <select value={role} onChange={(e) => setRole(e.target.value)}
            ...
            <option value="visualizador">Visualizador (sÃ³ leitura)</option>
      ...
      <div className="flex flex-col gap-2">
        <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>UsuÃ¡rios vinculados</p>
        {members === null ? (
          <Loader2 size={18} className="animate-spin" style={{ color: 'var(--accent)' }} />
        ) : members.length === 0 ? (
          <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>Nenhum usuÃ¡rio vinculado ainda.</p>
        ) : (
          members.map((m) => (
            <div key={m.id} className="flex items-center justify-between gap-2 rounded-lg p-2.5" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <IconButton title="Remover vÃ­nculo" danger onClick={() => removeMember(m.id)}><Trash2 size={14} /></IconButton>
              </div>
            </div>
          ))
        )}
...
function PageStyles() {
  return (
    <style>{`
      ...
      /* ---- Identidade de marca no RVT (cabeÃ§alho, cards, rodapÃ©) ---- */
      .rvt-brand-band {
      ...
      /* ---- Fundo da tela de login: emblema real como marca d'Ã¡gua no canto (mescla v1 + v6) ---- */
...
function Root() {
  ...
  // Um usuÃ¡rio que nÃ£o Ã© dono da plataforma sÃ³ enxerga os clientes aos quais foi vinculado (memberships)
  ...
  function selectClient(id) {
    ...
    // O antigo PIN por cliente sÃ³ se aplica quando o Supabase nÃ£o estÃ¡ configurado (modo local legado);
    // com Supabase, o login jÃ¡ foi validado antes de chegar aqui.
  ...
  // UsuÃ¡rio vinculado a um Ãºnico cliente entra direto nele, sem precisar escolher
...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    console.error('Erro na aplicaÃ§Ã£o:', error, info);
  }
  render() {
    if (this.state.error) {
      return (
