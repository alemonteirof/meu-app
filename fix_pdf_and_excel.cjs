...
function literalReplace(label, alvo, novo) {
  ...
  if (count === 0) { problemas.push(`[${label}] não achei: "${alvo.slice(0, 70)}..."`); return; }
  if (count > 1) { problemas.push(`[${label}] achei ${count}x (esperava 1): "${alvo.slice(0, 70)}..."`); return; }
